...
"""
Final, analysis-ready output stage: pulls together the enriched persons,
activities, trips, vehicles, and spatial locations produced by the rest of
the pipeline and writes them out as the dhaka_1pct_*.csv / *.gpkg files under
output_path (see output_prefix in config_dhaka.yml - the "ile_de_france_"
default below is never actually used for Dhaka). This is the stage aliased
to synthesis.output, the top-level target in config_dhaka.yml's run: list -
i.e. running the whole pipeline means running this stage and everything it
transitively depends on.

The `if context.config("mode_choice", False): context.stage("matsim.simulation.prepare")`
branch below never actually fires for Dhaka: config_dhaka.yml sets
mode_choice: False, and matsim.simulation.prepare/run are not wired up for
Dhaka anyway (see their own header comments - they need eqasim-java classes
that only exist for Seville's mode set). The "mode" column written into
trips.csv comes from dhaka.synthesis.population.mode_choice - a discrete
choice model fitted on the DTCA HTS and applied to each synthetic trip's
real assigned distance and traveler covariates (see that stage's docstring)
- NOT the raw mode inherited from the matched HTS donor trip anymore, and
still NOT a MATSim-simulated dynamic mode-choice result: it is a static,
one-shot assignment made at synthesis time, not re-chosen during MATSim's
...
def configure(context):
    ...
    context.stage("dhaka.synthesis.population.mode_choice")
...
def execute(context):
    ...
    # Prepare trips
    df_trips = context.stage("dhaka.synthesis.population.mode_choice").rename(
        columns = {
    ...
    df_trips = df_trips[[
        ...
        "is_first", "is_last",
        "euclidean_distance",
        # Modeled mode (dhaka.synthesis.population.mode_choice) - this is
        # the same mode already written into population.xml.gz's <leg>
        # elements. With mode_choice: False it is a fixed initial
        # assignment made once at synthesis time, not a MATSim-simulated
        # dynamic choice; kept here so it's visible without having to parse
        # the XML. mode_hts_donor is the pre-model baseline (the matched
        # HTS donor's mode) kept alongside for comparison/validation.
        "mode", "mode_hts_donor",
    ]]
