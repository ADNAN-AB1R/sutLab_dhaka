...
section for why. Calibration runs in **two stages**.

### 6a. Offline pre-calibration — seconds, run once

```bash
python -m dhaka.mode_choice.precalibrate_asc [--dry-run]
```

Solves the ASCs analytically against the same target shares using a
travel-time proxy, so the multi-hour MATSim rounds only have to correct the
simulation-dependent residual instead of the whole gap. Already applied —
re-run it after any change to `fallback_speed_kmh`, the fare assumptions, or
the trip population. Writes `dhaka_mode_parameters.json` in place.

Exact for teleported modes; blind to congested `car` time, routed `pt` time,
tour-level choice and congestion feedback — hence stage 6b.

### 6b. In-simulation calibration — iterative, hours per round
...
| `dhaka/mode_choice/precalibrate_asc.py` | Stage 1 ASC calibration — offline/analytic, seconds, run before the first MATSim round |
| `dhaka/mode_choice/calibrate_asc.py` | Stage 2 ASC calibration — against a real run's `modestats.csv`, iterative |
...
- Changing a `teleportedModeSpeed` in `assemble_scenario.py` without updating the matching `fallback_speed_kmh` in `dhaka_mode_parameters.json` (or vice versa). They must stay consistent, and the conversion is **not** 1:1 — MATSim scales beeline distance by `beelineDistanceFactor` (1.3) first, so `fallback_speed_kmh = teleportedModeSpeed / 1.3 * 3.6`. Getting this wrong silently biases every ASC calibrated against the proxy. Re-run `precalibrate_asc.py` after any such change.
- Using the trips CSV's `euclidean_distance` as the synthetic trip distance — it is the HTS *donor's* distance (r = 0.02 against the real one). The real distance is the `dhaka_1pct_trips.gpkg` geometry's length.
